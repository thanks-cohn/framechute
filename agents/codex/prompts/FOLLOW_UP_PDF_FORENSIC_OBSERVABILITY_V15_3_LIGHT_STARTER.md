...
Then inspect **PR #68** in full and continue the implementation from its current branch/head. Build on the existing PR #68 stable-ID, geometry, current-version, mask-ownership, semantic-search, Save/reopen, reconciliation, and diagnostics work rather than starting over.
...
Update **PR #68 itself** with the implementation and run every test/check required by the authoritative prompt. Do not create a disconnected replacement PR unless technically unavoidable.
