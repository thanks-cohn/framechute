...
const SELECT_MODE_KEY = "framechute.select-mode.v1";
...
async function resizeOptions(item, selectedCount = 1) {
  const image=imageOf(item),panel=document.createElement("div");panel.className="resize-panel";panel.innerHTML=`<div class="resize-controls"><label>Apply to <select name="scope"><option value="this">This image</option><option value="selected" ${selectedCount<2?"disabled":""}>Selected images (${selectedCount})</option></select></label><label>Width <input name="width" type="number" min="1" value="${image.naturalWidth}"></label><label>Height <input name="height" type="number" min="1" value="${image.naturalHeight}"></label><label><input name="lock" type="checkbox" checked> Preserve aspect ratio</label><label>Format <select name="format"><option value="png">PNG</option><option value="jpeg">JPEG</option><option value="webp">WebP</option></select></label><label><input name="save" type="checkbox" checked> Save As…</label><label><input name="add" type="checkbox"> Add result to Workspace</label><div><button type="button" data-scale=".5">50%</button> <button type="button" data-scale="2">200%</button></div></div><figure class="resize-preview"><img alt="Bounded proxy preview"><figcaption>Preparing preview…</figcaption></figure>`;
  ...
  const refresh=()=>{clearTimeout(timer);const current=++generation;timer=setTimeout(async()=>{try{const options=values(),scale=Math.min(1,1600/options.width,1200/options.height),proxy={...options,width:Math.max(1,Math.round(options.width*scale)),height:Math.max(1,Math.round(options.height*scale)),lockAspect:false},blob=await transformed(item,proxy);if(current!==generation)return;if(previewUrl)URL.revokeObjectURL(previewUrl);previewUrl=URL.createObjectURL(blob);preview.src=previewUrl;caption.textContent=`Requested output ${options.width} × ${options.height} px · proxy preview ${proxy.width} × ${proxy.height} px`;}catch(error){caption.textContent=`Preview unavailable: ${error.message}`;}},120);};
  ...
  const accepted=await showDialog("Img Size Change",panel,"Create result");clearTimeout(timer);generation++;if(previewUrl)URL.revokeObjectURL(previewUrl);if(!accepted)return null;if(!panel.querySelector('[name="save"]').checked&&!panel.querySelector('[name="add"]').checked)throw new Error("Choose Save As, Add to Workspace, or both.");return{...values(),scope:panel.querySelector('[name="scope"]').value,save:panel.querySelector('[name="save"]').checked,add:panel.querySelector('[name="add"]').checked};
}
...
register({ id: "image.resize", label: "Resize Image…", appliesTo: applies("image"), async run({ selection: items }) {
  const options=await resizeOptions(items[0],items.length);if(!options)return;const targets=options.scope==="selected"?items:[items[0]];
  await runBatch(targets,async item=>{const blob=await transformed(item,options),extension=options.format==="jpeg"?"jpg":options.format,filename=`${nameOf(item).replace(/\.[^.]+$/,"")}-${options.width}x${options.height}.${extension}`;if(options.save)await saveBlobAs({blob,filename,extension,mimeType:blob.type,description:"Resized image"});if(options.add)addResult(blob,filename);});
  announce(`${targets.length} image${targets.length===1?"":"s"} resized to ${options.width} × ${options.height}px. Originals unchanged.`);
} });
register({id:"image.copy",label:"Copy Image",appliesTo:applies("image",{max:1}),async run({selection:[item]}){if(!navigator.clipboard?.write||typeof ClipboardItem!=="function")throw new Error("Copy Image is unavailable because this browser has not granted image clipboard access.");const blob=await transformed(item,{format:"png"});await navigator.clipboard.write([new ClipboardItem({"image/png":blob})]);announce("Visible image copied to the clipboard.");}});
register({id:"image.open-tab",label:"Open Image in New Tab",appliesTo:applies("image",{max:1}),async run({selection:[item]}){const blob=await transformed(item,{format:"png"}),url=URL.createObjectURL(blob),opened=window.open(url,"_blank");if(!opened){URL.revokeObjectURL(url);throw new Error("The image tab was blocked. Allow pop-ups for FrameChute and try again.");}opened.opener=null;setTimeout(()=>URL.revokeObjectURL(url),60000);announce("Visible image opened in a new tab.");}});
...
register({ id: "image.save-as", label: "Save As", appliesTo: applies("image"), async run({ selection: items, progress }) {
  const panel=document.createElement("div");panel.className="export-panel";panel.innerHTML=`<label>Apply to <select name="scope"><option value="this">This image</option><option value="selected" ${items.length<2?"disabled":""}>Selected images (${items.length})</option></select></label><label>Format <select name="format"><option value="webp">WebP</option><option value="jpeg">JPEG</option><option value="png">PNG</option></select></label><label>Quality <input name="quality" type="range" min="10" max="100" value="86"><output>86%</output></label><label>Transparency background <input name="background" type="color" value="#ffffff"></label><p>The source remains unchanged. Encoded byte size is reported after export.</p>`;panel.querySelector('[name="quality"]').oninput=e=>panel.querySelector("output").value=`${e.target.value}%`;if(!await showDialog("Img Ext Change",panel,"Save As"))return;const targets=panel.querySelector('[name="scope"]').value==="selected"?items:[items[0]],format=panel.querySelector('[name="format"]').value,quality=Number(panel.querySelector('[name="quality"]').value)/100,background=format==="jpeg"?panel.querySelector('[name="background"]').value:undefined;
  const sizes=[],batchFiles={};await runBatch(targets, async (item) => { const blob = await transformed(item, { format, quality, background });let before=null;try{const response=await fetch(imageOf(item).currentSrc||imageOf(item).src);before=Number(response.headers.get("content-length"))|| (await response.blob()).size;}catch{}sizes.push([before,blob.size]);const extension=format==="jpeg"?"jpg":format,filename=`${nameOf(item).replace(/\.[^.]+$/,"")}.${extension}`;if(targets.length>1)batchFiles[filename]=new Uint8Array(await blob.arrayBuffer());else await saveBlobAs({ blob, filename, extension, mimeType: blob.type, description: "Image" }); }, { onProgress: progress });
  if(targets.length>1)await saveBlobAs({blob:new Blob([zipSync(batchFiles)],{type:"application/zip"}),filename:`converted-${format}-images.zip`,extension:"zip",mimeType:"application/zip",description:"Converted image batch"});
  if(targets.length===1)announce(`${sizes[0][0]?.toLocaleString()||"Unknown source size"} → ${sizes[0][1].toLocaleString()} bytes. Source unchanged.`);else announce(`${targets.length} images encoded with one shared configuration.`);
} });
...
let selectMode=false;try{selectMode=localStorage.getItem(SELECT_MODE_KEY)==="true";}catch{}
const selectButton=document.createElement("button");selectButton.type="button";selectButton.className="toolbar-command select-mode-toggle";selectButton.setAttribute("aria-pressed",String(selectMode));
function paintSelectMode(){document.body.classList.toggle("select-mode",selectMode);selectButton.textContent=`Select Mode  [ ${selectMode?"ON":"OFF"} ]`;selectButton.title=selectMode?"Clicks toggle objects without moving or editing them":"Turn on to select several media objects safely";}
selectButton.onclick=()=>{selectMode=!selectMode;try{localStorage.setItem(SELECT_MODE_KEY,String(selectMode));}catch{}paintSelectMode();};
(document.querySelector(".toolbar-primary")||document.querySelector(".toolbar"))?.append(selectButton);paintSelectMode();
workspace.addEventListener("pointerdown",event=>{if(!selectMode)return;const block=event.target.closest(".block");if(!block)return;event.preventDefault();event.stopImmediatePropagation();selection.toggle(block);},{capture:true});
workspace.addEventListener("click",event=>{if(!selectMode)return;event.preventDefault();event.stopImmediatePropagation();if(!event.target.closest(".block"))selection.clear();},{capture:true});
