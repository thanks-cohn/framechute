...
register({ id: "object.duplicate", label: "Clone", appliesTo: (s) => s.length > 0, async run({ selection: items }) {
...
register({ id: "image.save-as", label: "Save As…", appliesTo: applies("image"), async run({ selection: items, progress }) {
  const panel=document.createElement("div");panel.className="export-panel";panel.innerHTML='<label>Format <select name="format"><option value="webp">WebP</option><option value="jpeg">JPEG</option><option value="png">PNG</option></select></label><label>Quality <input name="quality" type="range" min="10" max="100" value="86"><output>86%</output></label><label>Transparency background <input name="background" type="color" value="#ffffff"></label><label><input name="keep-modified-size" type="checkbox"> Keep Modified Size</label><p>Off preserves original/native pixel dimensions. On uses dimensions from an explicit canvas resize; zoom, maximize, centering, and automatic fitting are ignored.</p>';panel.querySelector('[name="quality"]').oninput=e=>panel.querySelector("output").value=`${e.target.value}%`;if(!await showDialog(items.length>1?`Convert/compress ${items.length} images`:"Convert / compress image",panel,"Save As"))return;const format=panel.querySelector('[name="format"]').value,quality=Number(panel.querySelector('[name="quality"]').value)/100,background=format==="jpeg"?panel.querySelector('[name="background"]').value:undefined,keepModifiedSize=panel.querySelector('[name="keep-modified-size"]').checked;
  const sizes=[],batchFiles={};await runBatch(items, async (item) => { const width=Number(item.dataset.userResizedContentWidth),height=Number(item.dataset.userResizedContentHeight),outputSize=keepModifiedSize&&width>0&&height>0?{width,height,lockAspect:false}:{},blob = await transformed(item, { format, quality, background, ...outputSize });let before=null;try{const response=await fetch(imageOf(item).currentSrc||imageOf(item).src);before=Number(response.headers.get("content-length"))|| (await response.blob()).size;}catch{}sizes.push([before,blob.size]);const extension=format==="jpeg"?"jpg":format,filename=`${nameOf(item).replace(/\.[^.]+$/,"")}.${extension}`;if(items.length>1)batchFiles[filename]=new Uint8Array(await blob.arrayBuffer());else await saveBlobAs({ blob, filename, extension, mimeType: blob.type, description: "Image" }); }, { onProgress: progress });
