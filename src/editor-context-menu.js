...
import { isQuickActionsHidden, readQuickActionsEnabled, setQuickActionsHidden, writeQuickActionsEnabled } from "./actions/object-menu-model.mjs";
...
function run(action,value){const block=context?.block;if(!block)return;const selected=context.selected;
  if(action==="show-header")window.dispatchEvent(new CustomEvent("framechute:object-command",{detail:{block,command:"show-header"}}));
    ...
    if(visible){
      setQuickActionsHidden(block,true);
      window.dispatchEvent(new CustomEvent("framechute:quick-actions-object-changed",{detail:{block,hidden:true}}));
    } else {
      if(!globallyEnabled){
        const enabled=writeQuickActionsEnabled(true);
        window.dispatchEvent(new CustomEvent("framechute:quick-actions-global-changed",{detail:{enabled}}));
      }
      setQuickActionsHidden(block,false);
      window.dispatchEvent(new CustomEvent("framechute:quick-actions-object-changed",{detail:{block,hidden:false,show:true}}));
    }
  }
...
workspace?.addEventListener("contextmenu",event=>{const next=resolveEditorContext(event.target);if(!next)return;event.preventDefault();event.stopImmediatePropagation();next.quickActionsEnabled=readQuickActionsEnabled();next.quickActionsVisible=next.quickActionsEnabled&&!isQuickActionsHidden(next.block);next.clientX=event.clientX;next.clientY=event.clientY;const selection=document.getSelection();if(next.editorKind==="docx"&&selection?.rangeCount&&next.surface.contains(selection.anchorNode))next.range=selection.getRangeAt(0).cloneRange();if(next.selected?.matches(".pdf-text-item"))next.selected.dispatchEvent(new MouseEvent("click",{bubbles:true}));show(next,event.clientX||next.surface.getBoundingClientRect().left+24,event.clientY||next.surface.getBoundingClientRect().top+24);},true);
