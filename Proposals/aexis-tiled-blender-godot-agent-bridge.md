...
For artists who draw rather than model, let a character stay visually flat like a paper figure while a rig underneath carries joints, pivots, hit regions and depth anchors. Drag the visible hand, jaw, eye or torso to edit motion through that rig. At chosen moments a labeled part of the animated drawing can protrude into 3D, making the 2D Toonworld feel as though it is entering the spatial world. Bind selected art regions to depth limits, facing, render order, collision and a return pose so the effect is repeatable and adjustable. A simple rigged example comes before automatic rigging or free-form deformation; all of this is proposed work.
